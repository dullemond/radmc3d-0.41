...
strout='_0'+string(nout,format='(i4)')
if(nout lt 1000) then strout='_00'  +string(nout,format='(i3)')
...
openw,lun_t,/get_lun,'microturbulence'+strout+'.inp'
printf,lun_t,'1'           ; Format
printf,lun_t,nrcells       ; Number of leaf cells
printf,lun_t,'1'           ; Number of dust specy
...
openw,lun_v,/get_lun,'gas_velocity'+strout+'.inp'
printf,lun_v,'1'          ; Format
printf,lun_v,nrcells      ; Number of leaf cells
...
;=================================================
      ...
      vx = (*hydro.levelh(ilevel,son_cpu)).u(gson(0),ison,1)
      vy = (*hydro.levelh(ilevel,son_cpu)).u(gson(0),ison,2)
      vz = (*hydro.levelh(ilevel,son_cpu)).u(gson(0),ison,3)
      ...
      tp   = tp   * unit_tp
      vx = vx *info.unit_l/info.unit_t
      vy = vy *info.unit_l/info.unit_t
      vz = vz *info.unit_l/info.unit_t
      cs = (1.66667*kk*Tp/(mu*mp))^0.5

      printf,lun_amr,'0';,nbcell
      printf,lun_d,dens/gastodust,FORMAT = '(e12.5)'
      printf,lun_tp,tp,FORMAT = '(e12.5)'
      printf,lun_t,cs,FORMAT = '(e12.5)'
      printf,lun_v,vx,vy,vz,FORMAT = '(3(e12.5,2X))'
