...
import numpy as np
#
# Import struct for handling binary I/O
#
import struct
#
...
#
# Write the density file as binary
#
ncells = nx*ny*nz                        # The total nr of cells
idum   = np.array([1,8,ncells,1],dtype='int64') # Create an array of integers for the header
#                                        # First 1 is the format number, 8 means: double precision
#                                        # ncells = nr of cells, last 1 means 1 dust species
strf   = str(ncells)+'d'                 # Create a format string for struct
data   = rhod.ravel(order='F')           # Create a 1-D view, fortran-style indexing
sdat   = struct.pack('4Q',*idum)+struct.pack(strf,*data) # Create a binary image of the data
with open('dust_density.binp','w+b') as f:
