...
with open('midplane_temp_tau1e4_isoscat_mcfost','r') as f:
    data = np.loadtxt(f)
pinte_r, pinte_mid_temp = data.T

with open('vert0.2_temp_tau1e4_isoscat_mcfost','r') as f:
    data = np.loadtxt(f)
pinte_vert_02_z, pinte_vert_02_temp = data.T

with open('vert200_temp_tau1e4_isoscat_mcfost','r') as f:
    data = np.loadtxt(f)
...
with open('vert0.2_temp_tau1e4_isoscat_radmc3d','w') as f:
    for iz in range(0,nnt/2):
        f.write("%13.6e %13.6e\n"%(zcyl02[iz],tdust02[iz]))

with open('vert200_temp_tau1e4_isoscat_radmc3d','w') as f:
    for iz in range(0,nnt/2):
