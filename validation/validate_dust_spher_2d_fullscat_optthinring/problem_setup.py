...
yihalf   = math.pi/2.0 - zmaxr*np.linspace(nyhalf,0,nyhalf+1)/(nyhalf*1.0)
yi       = np.hstack((yihalf,math.pi-yihalf[:-1][::-1]))
...
zzr      = math.pi/2.0 - tt
rhod     = rho0 * np.exp(-0.50*(zzr/hpr)**2-0.5*((rr-rring)/(rring*hpr))**2)
#
...
with open('dustopac.inp','w+') as f:
    ...
    f.write('mix             Extension of name of dustkapscatmat_***.inp file\n')
    f.write('----------------------------------------------------------------------------\n')
#
...
with open('radmc3d.inp','w+') as f:
    f.write('nphot = %d\n'%(nphot))
    f.write('scattering_mode_max = 5\n')   # Put this to 5 for full anisotropic scattering
